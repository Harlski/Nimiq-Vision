...
function onTransaction(tx) {
    ...
    console.log("everything in tx:::");
    console.log(tx);
    console.log("txdataraw");
    console.log(tx.data.raw)
    console.log("txdata");
    console.log(tx.data);
    if (tx.state === "pending") {
        ...
        console.log("everything in decoded");
        console.log("message")
		if (message === ''){
			message = "Good Work!";
            }	
        ...
        queuePopup(removedSpaces, message, valueConverted, timeToWait); //Send .svg image, transaction msg, transaction value & a wait duration
